...
app.post("/additional", function (req, res) {
  User.findOneAndUpdate(
    ...
    { phone: req.body.phoneNo, address: req.body.addressTextarea },
    { upsert: true },
    function (err) {
      if (err) {
        console.log(err);
...
app.post("/editPassword", function (req, res) {
  if (req.isAuthenticated()) {
    if (
      ...
      req.user.password === req.body.old - password
    ) {
...
app.post("/paynow", function (req, res) {
  ...
  const callbackUrl = `/callback?orderID=${params.ORDER_ID}&amount=${params.TXN_AMOUNT}&email=${params.EMAIL}`;
...
<body style="text-align:center">
    ...
    <h2>Making Fake Payment</h2>
    <button style="background:#0099ff; color:white; padding: 20px 90px; border-radius:10px; border:0; font-size:large; font-weight:400"
    onClick='window.location.href="${callbackUrl}"'>Making Fake Payment</button>
</body>
</html>`;
  res.write(x);
  res.end();
  // res.send("Making Fake Payment")
